...
function playPassage(startVerseId, endVerseId) {
    ...
    $.getJSON('/ajax/passageAudio.xhtml', params, function(json) {
        playPassage2(json);
        $('#listen_' + startVerseId.toString() + '_' + endVerseId.toString()).html('<a href="javascript:stopPassage(' + startVerseId.toString() + ',' + endVerseId.toString() + ');"><img src="/img/stop.gif" border="0" alt="Stop"></a> ');
    });
}
...
function initListenElement(el) {
    parts = el.id.split('_');
    el.innerHTML = "<a href=\"javascript:playPassage(" + parts[1] + "," + parts[2] + ");\"><img src=\"/img/play.gif\" border=\"0\" alt=\"Listen\"></a> ";
}

function initVotingElement(el) {
    parts = el.id.split('_');
    if (document.all) votes = el.innerText; else votes = el.textContent;
    el.innerHTML = "<span class=\"vote\"><a href=\"javascript:vote('" + parts[1] + "'," + parts[2] + ",true)\"><img src=\"/img/voteup.png\" /></a><span class=\"inform\">" + votes + "</span><a href=\"javascript:vote('" + parts[1] + "'," + parts[2] + ",false)\"><img src=\"/img/votedown.png\" /></a></span>";
}
